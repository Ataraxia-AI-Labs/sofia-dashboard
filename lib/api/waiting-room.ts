...
export async function checkIn(
  ...
  const res = await authFetch(`${API_URL}/waiting-room/${orgId}/check-in`, {
    method: 'POST',
...
export async function getQueue(orgId: string): Promise<WaitingRoomEntry[]> {
  const res = await authFetch(`${API_URL}/waiting-room/${orgId}/queue`)
  if (!res.ok) return []
  return res.json()
}

export async function getLatePatients(orgId: string): Promise<LatePatient[]> {
  const res = await authFetch(`${API_URL}/waiting-room/${orgId}/late`)
  if (!res.ok) return []
  return res.json()
}

export async function notifyLate(orgId: string, patientId: string): Promise<void> {
  const res = await authFetch(`${API_URL}/waiting-room/${orgId}/notify-late`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function offerReschedule(orgId: string, appointmentId: string): Promise<void> {
  const res = await authFetch(`${API_URL}/waiting-room/${orgId}/offer-reschedule`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function notifyNext(orgId: string, patientId: string): Promise<void> {
  const res = await authFetch(`${API_URL}/waiting-room/${orgId}/notify-next`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ patient_id: patientId }),
  })
...
export async function completeVisit(orgId: string, appointmentId: string): Promise<void> {
  const res = await authFetch(`${API_URL}/waiting-room/${orgId}/complete`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ appointment_id: appointmentId }),
  })
...
export async function getWaitingStats(orgId: string): Promise<WaitingRoomStats | null> {
  const res = await authFetch(`${API_URL}/waiting-room/${orgId}/stats`)
