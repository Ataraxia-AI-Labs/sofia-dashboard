...
function StatCard({ label, value, sub, color }: { label: string; value: string; sub: string; color: string }) {
  return (
    <div className="glass-card p-3">
      <div className="text-[9px] font-mono font-semibold text-text-dim uppercase tracking-wider mb-1">{label}</div>
      <div className={`text-lg font-bold font-mono ${color}`}>{value}</div>
      <div className="text-[9px] font-mono text-text-dim mt-0.5">{sub}</div>
    </div>
  )
}
  ...
  return (
    <div className="max-w-[1200px] space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-sm font-mono font-bold uppercase tracking-wide text-text-primary">Latency Metrics</h2>
      ...
      <div className="flex items-center gap-4 text-[9px] font-mono text-text-dim">
        ...
        <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-status-danger" /> 500ms+ (critico)</div>
      </div>
      ...
      <div className="glass-card overflow-hidden">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          ...
          {slowCount > 0 && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-status-danger/10 border border-status-danger/20">
              <Activity size={10} className="text-status-danger" />
              <span className="text-[10px] font-mono font-semibold text-status-danger">{slowCount} critico{slowCount !== 1 ? 's' : ''}</span>
            </div>
          )}
        </div>
          ...
          <table className="w-full min-w-[700px]">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left px-4 py-2 text-[10px] font-mono font-semibold text-text-dim uppercase tracking-wider">Endpoint</th>
                <th className="text-left px-4 py-2 text-[10px] font-mono font-semibold text-text-dim uppercase tracking-wider">Metodo</th>
                <th className="text-right px-4 py-2 text-[10px] font-mono font-semibold text-text-dim uppercase tracking-wider">P50</th>
                <th className="text-right px-4 py-2 text-[10px] font-mono font-semibold text-text-dim uppercase tracking-wider">P95</th>
                <th className="text-right px-4 py-2 text-[10px] font-mono font-semibold text-text-dim uppercase tracking-wider">P99</th>
                <th className="text-right px-4 py-2 text-[10px] font-mono font-semibold text-text-dim uppercase tracking-wider">Avg</th>
                <th className="text-right px-4 py-2 text-[10px] font-mono font-semibold text-text-dim uppercase tracking-wider">Requests</th>
                <th className="text-center px-4 py-2 text-[10px] font-mono font-semibold text-text-dim uppercase tracking-wider">Historial</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-4 py-10 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-text-dim text-xs font-mono">Sin datos de latencia disponibles</p>
                    </div>
                  </td>
                </tr>
              ) : (
