    ...
    return (
      <div className="min-h-screen bg-void flex items-center justify-center">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-text-dim text-[12px] font-body">Cargando tu portal...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-void">
      <div className="max-w-lg mx-auto px-4 py-5 pb-20">
        ...
        <div className="text-center mb-5">
          ...
          <p className="text-[10px] text-brand-purple font-body font-medium uppercase tracking-wider mb-0.5">{data.clinic_name}</p>
          ...
          {data.gamification.next_tier && data.gamification.points_to_next_tier > 0 && (
            <div className="mt-2 max-w-xs mx-auto">
              <div className="flex items-center justify-between text-[12px] font-body text-text-dim mb-0.5">
              ...
              <p className="text-[12px] font-body text-text-dim mt-0.5 text-center">
            ...
            {otherAppointments.map(apt => (
              <div key={apt.id} className="p-3 rounded-lg bg-surface border border-border">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-body font-medium text-text-primary capitalize">{formatDate(apt.date)}</p>
                    <p className="text-[12px] font-body text-text-muted">{apt.time} - {apt.service} con {apt.doctor}</p>
                  </div>
                ...
                {confirmCancel === apt.id && (
                  <div className="flex gap-2 mt-1.5 pt-1.5 border-t border-border">
                    <button
                      ...
                      className="flex-1 py-1 rounded-md bg-status-danger/10 text-status-danger text-[12px] font-body font-medium disabled:opacity-50"
                    >
                      {cancellingId === apt.id ? 'Cancelando...' : 'Confirmar cancelacion'}
                    </button>
                    <button
                      onClick={() => setConfirmCancel(null)}
                      className="flex-1 py-1 rounded-md bg-surface-2 text-text-muted text-[12px] font-body font-medium"
                    >
                      Mantener
                    </button>
                  </div>
        ...
        <div className="mb-3 p-4 rounded-lg bg-surface border border-border">
          <h2 className="text-xs font-body font-bold text-text-primary flex items-center gap-2 mb-2.5">
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            <div className="text-center p-2.5 rounded-lg bg-brand-purple/8 border border-brand-purple/15">
              ...
              <p className="text-[12px] font-body text-text-dim">Puntos</p>
            </div>
            <div className="text-center p-2.5 rounded-lg bg-brand-gold/8 border border-brand-gold/15">
              ...
              <p className="text-[12px] font-body text-text-dim">Nivel</p>
            </div>
            <div className="text-center p-2.5 rounded-lg bg-status-warning/8 border border-status-warning/15">
              ...
              <p className="text-[12px] font-body text-text-dim">Meses racha</p>
            </div>
          </div>
          ...
          {data.gamification.recent_actions.length > 0 && (
            <div>
              <p className="text-[12px] font-body text-text-dim font-semibold uppercase tracking-wider mb-1.5">Actividad reciente</p>
              <div className="space-y-1">
                {data.gamification.recent_actions.slice(0, 6).map((a, i) => (
                  <div key={i} className="flex items-center justify-between py-1 px-2.5 rounded-md bg-surface-2">
                    <span className="text-[12px] font-body text-text-muted">{a.action.replace(/_/g, ' ').toLowerCase()}</span>
                    <span className="text-[12px] font-body font-bold text-status-success">+{a.points}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {pendingPayments.length > 0 && (
          <div className="mb-3 p-4 rounded-lg bg-status-warning/8 border border-status-warning/20">
            <h2 className="text-xs font-body font-bold text-status-warning flex items-center gap-2 mb-2.5">
            ...
            <div className="space-y-1.5">
              {pendingPayments.map(p => (
                <div key={p.id} className="flex items-center justify-between py-1.5 px-2.5 rounded-lg bg-surface border border-border">
                  <div>
                    <p className="text-[12px] font-body font-medium text-text-primary">{p.description}</p>
                    <p className="text-[12px] font-body text-text-dim">{formatShortDate(p.date)}</p>
                  </div>
                  <span className="text-xs font-body font-bold text-status-warning">{formatCOP(p.amount)}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-1">
              {paidPayments.slice(0, 5).map(p => (
                <div key={p.id} className="flex items-center justify-between py-1.5 px-2.5 rounded-md bg-surface-2">
                  <div>
                    <p className="text-[12px] font-body text-text-muted">{p.description}</p>
                    <p className="text-[12px] font-body text-text-dim">{formatShortDate(p.date)}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-[12px] font-body font-semibold text-text-primary">{formatCOP(p.amount)}</span>
                    <p className="text-[12px] font-body font-medium text-status-success">Pagado</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {data.appointment_history.length > 0 && (
          <div className="mb-3 p-4 rounded-lg bg-surface border border-border">
            ...
            >
              <h2 className="text-xs font-body font-bold text-text-primary flex items-center gap-2">
                ...
                {data.appointment_history.map((h, i) => (
                  <div key={i} className="flex items-center gap-2.5 py-1.5">
                    ...
                    <div className="flex-1">
                      <p className="text-[12px] font-body font-medium text-text-primary">{h.service}</p>
                      <p className="text-[12px] font-body text-text-dim">{formatShortDate(h.date)} - Dr. {h.doctor}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="mb-3 p-4 rounded-lg bg-brand-purple/8 border border-brand-purple/15">
          <h2 className="text-xs font-body font-bold text-brand-purple flex items-center gap-2 mb-1.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="text-center p-1.5 rounded-md bg-surface/70 border border-border">
              ...
              <p className="text-[12px] font-body text-text-dim">Referidos</p>
            </div>
            <div className="text-center p-1.5 rounded-md bg-surface/70 border border-border">
              <p className="text-lg font-bold font-mono text-brand-purple">{data.referral.discounts_earned}</p>
              <p className="text-[12px] font-body text-text-dim">Descuentos ganados</p>
            </div>
          </div>
        </div>
