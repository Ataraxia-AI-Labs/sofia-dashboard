    ...
    <div className="space-y-4">
      ...
      <div className="glass-card p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-brand-purple/15 to-status-warning/15 border border-brand-purple/10 flex items-center justify-center">
          ...
          <button
            ...
            disabled={analyzing}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-brand-purple to-brand-purple-dark text-white font-semibold text-xs transition-all hover:shadow-lg hover:shadow-brand-purple/20 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {analyzing ? (
              <>
        ...
        {suggestions.length > 0 && (
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-void/50 rounded-lg p-3 text-center">
              ...
              <div className="text-[10px] text-text-dim">{t('statusLabels.pending')}</div>
            </div>
            <div className="bg-void/50 rounded-lg p-3 text-center">
              <div className="text-lg font-bold text-status-success font-mono">{appliedCount}</div>
              <div className="text-[10px] text-text-dim">{t('statusLabels.applied')}</div>
            </div>
            <div className="bg-void/50 rounded-lg p-3 text-center">
              <div className="text-lg font-bold text-status-danger font-mono">{rejectedCount}</div>
              <div className="text-[10px] text-text-dim">{t('statusLabels.rejected')}</div>
            </div>
          </div>
        )}
      ...
      {loading ? (
        ...
        <div className="glass-card p-8 text-center">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-purple/10 to-brand-cyan/10 border border-brand-purple/10 flex items-center justify-center mx-auto mb-4">
...
function SuggestionCard({
  ...
  return (
    <div className="glass-card p-5 animate-fade-in">
      ...
      {/* Reasoning */}
      <div className="mb-4 px-3 py-2.5 rounded-lg bg-surface-3 border border-border">
      ...
      {suggestion.status === 'PENDING' ? (
        <div className="flex items-center justify-end gap-2 pt-3 border-t border-border">
          ...
          <button
            ...
            disabled={actionLoading !== null}
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-[11px] font-semibold text-white bg-gradient-to-r from-status-success to-status-success/80 hover:shadow-md hover:shadow-status-success/20 transition-all disabled:opacity-50"
          >
            {actionLoading === 'apply' ? (
              <Loader2 size={11} className="animate-spin" />
            ) : (
