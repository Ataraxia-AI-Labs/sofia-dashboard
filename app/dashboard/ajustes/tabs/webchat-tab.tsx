    ...
    <div className="space-y-4">
      ...
      <div className="glass-card p-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-[12px] font-body font-semibold uppercase tracking-wide text-text-primary flex items-center gap-1.5">
              <MessageCircle size={13} className="text-brand-purple" />
              Web Chat Widget
            </h3>
            <p className="text-[11px] font-body text-text-dim mt-0.5">{t('webchat.desc') || 'Widget embebible para tu sitio web'}</p>
          </div>
          <button
      ...
      <div className="glass-card p-4 space-y-3">
        <h4 className="text-[12px] font-body font-semibold text-text-primary flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-[11px] font-body text-text-dim uppercase tracking-wider block mb-1">{t('webchat.color') || 'Color primario'}</label>
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                onChange={e => setConfig(prev => ({ ...prev, primary_color: e.target.value }))}
                disabled={isReadOnly}
                className="flex-1 text-[12px] font-body bg-surface border border-border rounded px-2 py-1.5 text-text-primary"
              />
            </div>
          </div>

          <div>
            <label className="text-[11px] font-body text-text-dim uppercase tracking-wider block mb-1">{t('webchat.position') || 'Posición'}</label>
            <select
              value={config.position}
              onChange={e => setConfig(prev => ({ ...prev, position: e.target.value as WebchatConfig['position'] }))}
              disabled={isReadOnly}
              className="w-full text-[12px] font-body bg-surface border border-border rounded px-2 py-1.5 text-text-primary"
            >
        ...
        <div>
          <label className="text-[11px] font-body text-text-dim uppercase tracking-wider block mb-1">{t('webchat.bubbleText') || 'Texto del botón'}</label>
          <input
            type="text"
        ...
        <div>
          <label className="text-[11px] font-body text-text-dim uppercase tracking-wider block mb-1">{t('webchat.welcome') || 'Mensaje de bienvenida'}</label>
          <textarea
            value={config.welcome_message}
            onChange={e => setConfig(prev => ({ ...prev, welcome_message: e.target.value }))}
            disabled={isReadOnly}
            rows={2}
            className="w-full text-[12px] font-body bg-surface border border-border rounded px-2 py-1.5 text-text-primary resize-none"
          />
        </div>

        <div>
          <label className="text-[11px] font-body text-text-dim uppercase tracking-wider block mb-1">{t('webchat.autoOpen') || 'Auto-abrir después de (ms, 0 = desactivado)'}</label>
          <input
            type="number"
            value={config.auto_open_delay_ms}
            onChange={e => setConfig(prev => ({ ...prev, auto_open_delay_ms: parseInt(e.target.value) || 0 }))}
            disabled={isReadOnly}
            className="w-32 text-[12px] font-body bg-surface border border-border rounded px-2 py-1.5 text-text-primary"
          />
        </div>
      </div>
      ...
      {/* Security */}
      <div className="glass-card p-4 space-y-3">
        <h4 className="text-[12px] font-body font-semibold text-text-primary">{t('webchat.security') || 'Dominios permitidos'}</h4>
        <p className="text-[11px] font-body text-text-dim">{t('webchat.domainsHint') || 'Un dominio por línea. Dejar vacío para permitir todos.'}</p>
        <textarea
          value={domainsText}
      ...
      {/* Preview */}
      <div className="glass-card p-4">
        <h4 className="text-[12px] font-body font-semibold text-text-primary mb-3">{t('webchat.preview') || 'Vista previa'}</h4>
        ...
        <p className="mt-2 text-[10px] font-body text-text-dim uppercase tracking-wider">Asi se vera en el sitio web del cliente</p>
      </div>
      ...
      {!isReadOnly && (
        <button
          onClick={handleSave}
          disabled={saving}
          className="px-4 py-2 rounded-lg bg-brand-purple text-white text-[12px] font-body font-semibold hover:bg-brand-purple/90 transition-colors disabled:opacity-50"
        >
