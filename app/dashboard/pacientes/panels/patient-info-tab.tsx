  ...
  return (
    <>
      ...
      {patient.psychometrics && (
        <div className="glass-card p-4 space-y-3">
          <h4 className="text-xs font-mono font-semibold text-text-muted uppercase tracking-wider">Psicometria</h4>
      ...
      {treatments.length > 0 && (
        <div className="glass-card p-4 space-y-3">
          <h4 className="text-xs font-mono font-semibold text-text-muted uppercase tracking-wider">Tratamientos Activos</h4>
          {treatments.map((t) => (
            <div key={t.id} className={`bg-void/50 rounded-lg px-3 py-2 ${t.status !== 'ACTIVE' ? 'opacity-50' : ''}`}>
              <div className="flex justify-between items-center">
                <span className="text-xs font-mono font-semibold text-text-primary">{t.treatment_name}</span>
...
function DetailRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-xs font-mono text-text-muted w-20 flex-shrink-0">{label}</span>
...
function MiniMetric({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="bg-surface-3 rounded-md px-3 py-2">
      <div className="text-[10px] font-mono text-text-dim mb-0.5">{label}</div>
