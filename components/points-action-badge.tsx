  ...
  if (compact) {
    return (
      ...
      >
        ...
        {points != null && (
          <span className={`text-[11px] font-body font-bold ${cfg.textColor}`}>+{points}</span>
        )}
      </span>
    )
  }
  ...
  return (
    <div className={`inline-flex items-center gap-2 px-2 py-1 rounded ${cfg.bg} border ${cfg.border} ${className}`}>
      ...
      <div className="flex flex-col">
        <span className={`text-[12px] font-body font-semibold ${cfg.textColor}`}>{cfg.label}</span>
      </div>
      {points != null && (
        <span className={`text-[10px] font-bold font-body ${cfg.textColor} ml-auto`}>
