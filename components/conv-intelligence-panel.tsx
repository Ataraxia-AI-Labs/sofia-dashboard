...
export function ConvIntelligencePanel({ orgId, patientId, patientName }: Props) {
  ...
  if (loading) return <div className="p-3 text-[12px] font-body text-text-dim">...</div>
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden">
      <div className="bg-surface-2/50 px-3 py-2 border-b border-border/30">
        ...
        {patientName && <p className="text-[11px] font-body text-text-dim">{patientName}</p>}
      </div>
      ...
      <div className="flex border-b border-border/30">
        {([
          ...
          <button key={t.id} onClick={() => setActiveSection(t.id)}
            className={`flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-body border-b-2 transition-colors ${
      ...
      <div className="p-3 max-h-[300px] overflow-y-auto">
        ...
        {activeSection === 'summary' && (
          summary ? (
            <p className="text-[12px] font-body text-text-secondary leading-relaxed">{summary}</p>
          ) : (
            <p className="text-[12px] font-body text-text-dim">Sin resumen disponible</p>
          )
        )}
          ...
          emotions ? (
            <div className="space-y-2">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[11px] font-body text-text-dim">Dominante:</span>
                <span className={`text-[12px] font-body font-bold ${EMOTION_COLORS[emotions.dominant_emotion] || 'text-text-primary'}`}>
                  {emotions.dominant_emotion}
                </span>
                <span className="text-[11px] font-body text-text-dim">Estabilidad: {(emotions.emotional_stability * 100).toFixed(0)}%</span>
              </div>
              {Object.entries(emotions).filter(([k]) => k in EMOTION_COLORS).map(([emotion, score]) => (
                <div key={emotion} className="flex items-center gap-2">
                  <span className={`text-[11px] font-body w-20 ${EMOTION_COLORS[emotion]}`}>{emotion}</span>
                  ...
                  <span className="text-[11px] font-body text-text-dim w-8 text-right">{((score as number) * 100).toFixed(0)}%</span>
                </div>
              ))}
            </div>
          ) : <p className="text-[12px] font-body text-text-dim">Sin datos emocionales</p>
        )}
          ...
          personality ? (
            <div className="space-y-2">
              {personality.communication_language && (
                <p className="text-[11px] font-body text-text-dim mb-2">
              ...
              {([
                ...
                <div key={dim.key} className="flex items-center gap-2">
                  <span className="text-[11px] font-body text-text-muted w-28">{dim.label}</span>
                  <div className="flex-1 h-1.5 bg-surface-2 rounded-full">
                  ...
                  <span className="text-[11px] font-body text-text-dim w-8 text-right">{(((personality as unknown as Record<string, number>)[dim.key] || 0) * 100).toFixed(0)}%</span>
                </div>
              ))}
            </div>
          ) : <p className="text-[12px] font-body text-text-dim">Sin perfil de personalidad</p>
        )}

        {/* Memories */}
        {activeSection === 'memories' && (
          memories.length === 0 ? (
            <p className="text-[12px] font-body text-text-dim">Sin memorias almacenadas</p>
          ) : (
            <div className="space-y-2">
              {memories.map(m => (
                <div key={m.id} className="border border-border/50 rounded p-2">
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <span className="text-[10px] font-body bg-surface-2 px-1.5 py-0.5 rounded text-text-dim uppercase">{m.category}</span>
                    <span className="text-[10px] font-body text-text-dim">{m.source}</span>
                  </div>
                  <p className="text-[12px] font-body text-text-secondary">{m.content}</p>
                </div>
              ))}
            </div>
          )
