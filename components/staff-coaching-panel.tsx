...
export function StaffCoachingPanel({ orgId }: Props) {
  ...
  if (loading) return <div className="p-4 text-[10px] font-mono text-text-dim">...</div>
  ...
  return (
    <div className="border border-border rounded-lg">
      <div className="bg-surface-2/50 px-4 py-2.5 border-b border-border flex items-center justify-between">
        <p className="text-[11px] font-mono font-bold text-brand-purple flex items-center gap-1.5">
          <GraduationCap size={14} /> Coaching IA
        </p>
        {unreadTips.length > 0 && (
          <span className="text-[8px] font-mono bg-brand-purple/10 text-brand-purple px-1.5 py-0.5 rounded-full font-semibold">
      ...
      <div className="flex border-b border-border">
        <button onClick={() => setTab('tips')}
          className={`flex items-center gap-1 px-3 py-1.5 text-[10px] font-mono border-b-2 transition-colors ${
      ...
      <div className="p-3 max-h-[400px] overflow-y-auto">
        {tab === 'tips' ? (
          tips.length === 0 ? (
            <p className="text-[10px] font-mono text-text-dim py-4 text-center">Sin tips de coaching disponibles</p>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-1.5 mb-1">
                        <span className={`text-[8px] font-mono px-1.5 py-0.5 rounded font-semibold uppercase ${
                          tip.priority === 'HIGH' ? 'bg-status-danger/8 text-status-danger'
                          : tip.priority === 'MEDIUM' ? 'bg-status-warning/8 text-status-warning'
                          : 'bg-surface-2 text-text-dim'
                        }`}>{tip.priority}</span>
                        <span className="text-[8px] font-mono text-text-dim">{tip.category}</span>
                      </div>
                      <p className="text-[10px] font-mono text-text-secondary">{tip.tip}</p>
                    </div>
                    {!tip.is_read && (
          ...
          metrics.length === 0 ? (
            <p className="text-[10px] font-mono text-text-dim py-4 text-center">Sin metricas de staff</p>
          ) : (
            <div className="space-y-2">
              {metrics.map(m => (
                <div key={m.staff_id} className="border border-border rounded-lg p-3">
                  <p className="text-[11px] font-mono font-semibold text-text-primary">{m.staff_name}</p>
                  <div className="grid grid-cols-4 gap-2 mt-2">
                    <div>
                      <p className="text-[8px] font-mono text-text-dim">Conversaciones</p>
                      <p className="text-[11px] font-mono font-bold text-text-primary">{m.conversations_handled}</p>
                    </div>
                    <div>
                      <p className="text-[8px] font-mono text-text-dim">Resp. (s)</p>
                      <p className="text-[11px] font-mono font-bold text-text-primary">{m.avg_response_time.toFixed(0)}</p>
                    </div>
                    <div>
                      <p className="text-[8px] font-mono text-text-dim">Satisfaccion</p>
                      <p className="text-[11px] font-mono font-bold text-status-success">{(m.satisfaction_score * 100).toFixed(0)}%</p>
                    </div>
                    <div>
                      <p className="text-[8px] font-mono text-text-dim">Resolucion</p>
                      <p className="text-[11px] font-mono font-bold text-brand-purple">{(m.resolution_rate * 100).toFixed(0)}%</p>
                    </div>
                  </div>
                </div>
              ))}
